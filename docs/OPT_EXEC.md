# TASK: Execute Next Planned Item for Velocity Go Project
...
## CODE STANDARDS (Velocity-specific)

- **Architecture**: Follow existing Velocity package structure and conventions.
- **Determinism**: All procedural generation uses `rand.New(rand.NewSource(seed))`. Never `time.Now()` or global `math/rand` for stateful logic.
...
- **Testing**: Table-driven with `t.Run`. Use stubs where needed to avoid runtime dependencies. Target ≥40% coverage per package (≥30% for display/platform-dependent packages).
