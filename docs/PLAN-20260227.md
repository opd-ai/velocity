...
## Known Gaps
- **Procedural audio generation**: Ebitengine's audio API supports PCM streaming, but the exact approach for generating music layers procedurally (e.g., frequency tables, beat sequencing) is not specified in the roadmap. **All audio must be procedurally generated—no audio files are permitted.** See GAPS.md
- **Sprite generation algorithm**: The roadmap requires "procedurally drawn" sprites with no external assets, but the specific algorithm (pixel-mirroring, shape primitives, L-system) is not defined. **All sprites must be procedurally generated—no image files are permitted.** See GAPS.md
